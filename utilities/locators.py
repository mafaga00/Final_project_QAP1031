...
class AuthLocators:
    """Класс содержащий все локаторы для страницы авторизации."""
    ...
    USERNAME = (By.ID, 'username')  # поле ввода для телефона, почты, логина, лицевого счета
    PASSWORD = (By.ID, 'password')  # поле ввода для пароля
    BTN_LOGIN = (By.ID, 'kc-login')  # кнопка "Войти"
    VK_LINK = (By.ID, 'oidc_vk')  # кнопка авторизации с помощью "VK ID"
    OK_LINK = (By.ID, 'oidc_ok')  # кнопка авторизации с помощью "Одноклассники"
    MAIL_RU_LINK = (By.ID, 'oidc_mail')  # кнопка авторизации с помощью "Мой Мир"
    YA_LINK = (By.ID, 'oidc_ya')  # кнопка авторизации с помощью аккаунта "Яндекс"
